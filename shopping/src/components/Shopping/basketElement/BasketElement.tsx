...
const BasketElement = ({
  ...
  const handleChange = (e: React.ChangeEvent<{ value: unknown }>) => {
    ...
    dispatch(
      changeBasketItem(sizedProduct, e.target.value as number, productKey)
    );
  };
    ...
    <div className="basketElement">
      <div className="basketElement-imageWrapper">
        <Link to={"/product" + "/" + sizedProduct.id}>
          <img
            className="basketElement-image"
            src={sizedProduct.image}
            alt=""
          />
        </Link>
      </div>
      <div className="basketElement-dataWrapper">
        <div className="basketElement-upperData">
          ...
          <div>
            <IconButton
              className="basketElement-iconButton basketElement-materialComponent"
              onClick={handleClick}
            >
              <DeleteIcon />
            </IconButton>
        ...
        <div className="basketElement-lowerData">
          ...
          <p>
            Suma:{" "}
            {(sizedProduct.price * sizedProduct.quantity).toFixed(2) + " PLN"}
          </p>
          <Select
            className="basketElement-select basketElement-materialComponent"
            variant="outlined"
            id="basketElement-selectQuantity"
            value={sizedProduct.quantity}
            onChange={handleChange}
          >
            {possibleQuantities.map((productsQuantity) => (
              <MenuItem
                key={"quantity" + productsQuantity}
